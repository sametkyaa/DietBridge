...
-- placements default to server-authoritative Recipe master values, while an
-- explicit snapshot_mode = 'custom' opts into validated plan-only fields.
...
declare
  ...
  v_recipe_id uuid;
  v_recipe record;
  v_recipe_image_path text;
  v_use_payload_snapshot boolean;
  v_snapshot_mode text;
...
begin
  ...
  loop
    ...
    loop
      ...
      v_snapshot_mode := null;
      if v_meal ? 'snapshot_mode' then
        if jsonb_typeof(v_meal -> 'snapshot_mode') is distinct from 'string'
           or v_meal ->> 'snapshot_mode' not in ('recipe_master', 'custom') then
          raise exception 'snapshot_mode must be recipe_master or custom.' using errcode = '22023';
        end if;
        v_snapshot_mode := v_meal ->> 'snapshot_mode';
      end if;
      ...
      if v_source = 'recipe' then
        if v_meal_id is null then
          ...
          v_recipe_image_path := v_recipe.image_path;

          if coalesce(v_snapshot_mode, 'recipe_master') = 'custom' then
            -- Recipe ownership remains mandatory; only validated snapshot fields
            -- switch to the plan payload for this unsaved placement.
            ...
            v_photo_url := v_recipe_image_path;
        ...
        if v_meal_id is null and v_snapshot_mode is not null then
          raise exception 'snapshot_mode is only valid for new recipe meals.' using errcode = '22023';
        end if;
        if v_meal ? 'recipe_id' and jsonb_typeof(v_meal -> 'recipe_id') <> 'null' then
        ...
        if not (v_meal ? 'photo_url') or jsonb_typeof(v_meal -> 'photo_url') = 'null' then
          ...
          if v_photo_url ~ '^meal-plans/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.(jpe?g|png|webp)$' then
            ...
            null;
                ...
                and v_meal_id is null
                and coalesce(v_snapshot_mode, 'recipe_master') = 'custom'
                and v_recipe_image_path is not null
                and v_photo_url is not distinct from v_recipe_image_path
                and v_photo_url ~ '^recipes/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.(jpe?g|png|webp)$' then
            null;
          else
