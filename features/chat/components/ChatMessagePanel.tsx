...
const ChatMessagePanel: React.FC<ChatMessagePanelProps> = ({ conversation, messages, optimisticMessages, isLoading, isLoadingOlder, error, loadOlderError, receiptError, hasMore, onLoadOlder, onRetry, onRetryOptimistic, onDeleteMessage, onVisibleIncomingMessage, onBack, showBackButton, composer }) => {
  ...
  return <section className="relative flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" aria-label="Mesaj geçmişi">
    ...
    {receiptError && <div className="border-t border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900" role="alert">{receiptError}</div>}
