...
const runFlows = async () => {
  ...
  assert(sourceMigrations.at(-1) === '20260901200413_client_account_deletion_scope_tightening.sql', 'CANONICAL_MIGRATION_TAIL', sourceMigrations.at(-1));
  ...
  assert(disposable.disposableHistory.repositoryMigrationCount === 53, 'BASELINE_MATERIALIZED_COUNT_53');
  ...
  const runtimeFiles = readdirSync(runtimeMigrationDirectory)
    .filter((name) => /^\d+_.+\.sql$/.test(name))
    .sort();
  assert(runtimeFiles.length === 57, 'DISPOSABLE_MIGRATION_FILES_57_WITH_ONE_LOCAL_PREREQUISITE');
