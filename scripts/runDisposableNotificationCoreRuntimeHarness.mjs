...
const runFlows = async () => {
  const sourceMigrations = readdirSync(migrationDirectory)
    ...
    .sort();
  assert(sourceMigrations.length === 51, 'CANONICAL_MIGRATION_INVENTORY_51');
  assert(sourceMigrations.at(-1) === '20260830060342_dietitian_diploma_storage_hardening.sql', 'CANONICAL_MIGRATION_TAIL', sourceMigrations.at(-1));
  ...
  assert(disposable.disposableHistory.repositoryMigrationCount === 48, 'BASELINE_MATERIALIZED_COUNT_48');
  ...
  const runtimeFiles = readdirSync(runtimeMigrationDirectory)
    .filter((name) => /^\d+_.+\.sql$/.test(name))
    .sort();
  assert(runtimeFiles.length === 52, 'DISPOSABLE_MIGRATION_FILES_52_WITH_ONE_LOCAL_PREREQUISITE');
  ...
  assert(migrationCount === 52, 'DISPOSABLE_SCHEMA_MIGRATION_COUNT', 'canonical=51, local-prerequisite=1');
  pass('DISPOSABLE_CANONICAL_MIGRATION_REPLAY_51');
