...
const runFlows = async () => {
  ...
  assert(sourceMigrations.length === 53, 'REMINDER_CANONICAL_MIGRATION_INVENTORY_53');
  assert(sourceMigrations.at(-1) === '20260830185101_meal_plan_snapshot_edit_contract.sql', 'REMINDER_CANONICAL_MIGRATION_TAIL');
  ...
  assert(manifest.expectedHistory.total === 50, 'REMINDER_BASELINE_MATERIALIZED_COUNT_50');
  assert(runtimeFiles.length === 54, 'REMINDER_DISPOSABLE_MIGRATION_FILES_54');
  ...
  assert(migrationCount === 54, 'REMINDER_SCHEMA_MIGRATION_COUNT', 'canonical=53, local-prerequisite=1');
