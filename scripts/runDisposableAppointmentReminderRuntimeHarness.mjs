...
const runFlows = async () => {
  ...
  assert(sourceMigrations.length === 51, 'REMINDER_CANONICAL_MIGRATION_INVENTORY_51');
  assert(sourceMigrations.at(-1) === '20260830060342_dietitian_diploma_storage_hardening.sql', 'REMINDER_CANONICAL_MIGRATION_TAIL');
  ...
  assert(manifest.expectedHistory.total === 48, 'REMINDER_BASELINE_MATERIALIZED_COUNT_48');
  assert(runtimeFiles.length === 52, 'REMINDER_DISPOSABLE_MIGRATION_FILES_52');
  ...
  assert(migrationCount === 52, 'REMINDER_SCHEMA_MIGRATION_COUNT', 'canonical=51, local-prerequisite=1');
