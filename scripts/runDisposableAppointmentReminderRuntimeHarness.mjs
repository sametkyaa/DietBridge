...
const runFlows = async () => {
  ...
  assert(sourceMigrations.length === 55, 'REMINDER_CANONICAL_MIGRATION_INVENTORY_55');
  assert(sourceMigrations.at(-1) === '20260831190352_meal_completion_photo_contract.sql', 'REMINDER_CANONICAL_MIGRATION_TAIL');
  ...
  assert(manifest.expectedHistory.total === 52, 'REMINDER_BASELINE_MATERIALIZED_COUNT_52');
  assert(runtimeFiles.length === 56, 'REMINDER_DISPOSABLE_MIGRATION_FILES_56');
  ...
  assert(migrationCount === 56, 'REMINDER_SCHEMA_MIGRATION_COUNT', 'canonical=55, local-prerequisite=1');
