...
const runFlows = async () => {
  ...
  assert(sourceMigrations.length === 52, 'REMINDER_CANONICAL_MIGRATION_INVENTORY_52');
  assert(sourceMigrations.at(-1) === '20260830141202_meal_plan_cross_day_identity_preservation.sql', 'REMINDER_CANONICAL_MIGRATION_TAIL');
  ...
  assert(manifest.expectedHistory.total === 49, 'REMINDER_BASELINE_MATERIALIZED_COUNT_49');
  assert(runtimeFiles.length === 53, 'REMINDER_DISPOSABLE_MIGRATION_FILES_53');
  ...
  assert(migrationCount === 53, 'REMINDER_SCHEMA_MIGRATION_COUNT', 'canonical=52, local-prerequisite=1');
