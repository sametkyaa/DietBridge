...
const runFlows = async () => {
  ...
  assert(sourceMigrations.length === 54, 'REMINDER_CANONICAL_MIGRATION_INVENTORY_54');
  assert(sourceMigrations.at(-1) === '20260831071948_meal_plan_new_recipe_custom_snapshot_contract.sql', 'REMINDER_CANONICAL_MIGRATION_TAIL');
  ...
  assert(manifest.expectedHistory.total === 51, 'REMINDER_BASELINE_MATERIALIZED_COUNT_51');
  assert(runtimeFiles.length === 55, 'REMINDER_DISPOSABLE_MIGRATION_FILES_55');
  ...
  assert(migrationCount === 55, 'REMINDER_SCHEMA_MIGRATION_COUNT', 'canonical=54, local-prerequisite=1');
