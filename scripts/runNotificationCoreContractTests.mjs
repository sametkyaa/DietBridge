...
assert(files.length === 51, 'NOTIFICATION_MIGRATION_COUNT_51_WITH_DIPLOMA_STORAGE', 'count=' + files.length);
...
assert(files.at(-3) === '20260826133224_product_admin_dietitian_verification.sql', 'PRODUCT_ADMIN_MIGRATION_BEFORE_STANDALONE_ADMIN', files.at(-3));
assert(files.at(-2) === '20260827084741_standalone_platform_admin_access.sql', 'STANDALONE_ADMIN_MIGRATION_BEFORE_DIPLOMA_STORAGE', files.at(-2));
assert(files.at(-1) === '20260830060342_dietitian_diploma_storage_hardening.sql', 'DIPLOMA_STORAGE_MIGRATION_TAIL', files.at(-1));
