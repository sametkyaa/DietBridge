...
const assertManifestMatchesSourceInventory = ({ repoRoot, runtimeManifest }) => {
  if (runtimeManifest.expectedHistory?.canonical !== 45
      || runtimeManifest.expectedHistory?.image !== 7
      || runtimeManifest.expectedHistory?.total !== 52
      || runtimeManifest.files?.length !== 52) {
    throw new Error('Unexpected disposable migration inventory; expected 45 canonical and 7 image migrations.');
  }
...
const assertDisposableMigrationInventory = ({ repositoryPaths, tempRoot, localPrerequisite }) => {
  ...
  if (repositoryPaths.length !== 52 || disposablePaths.length !== 53) {
