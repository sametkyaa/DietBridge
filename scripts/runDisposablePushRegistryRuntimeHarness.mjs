...
const runFlows = async () => {
  ...
  assert(sourceMigrations.at(-1) === pushRegistryMigrationName, 'CANONICAL_PUSH_MIGRATION_TAIL');
  ...
  const runtimeFiles = readdirSync(runtimeMigrationDirectory)
    .filter((name) => /^\d+_.+\.sql$/.test(name))
    .sort();
  assert(runtimeFiles.length === 49, 'DISPOSABLE_MIGRATION_FILES_49');
  ...
  assert(countBySql('select count(*) from supabase_migrations.schema_migrations;') === 49, 'DISPOSABLE_MIGRATION_REPLAY_48_PLUS_PREREQUISITE');
