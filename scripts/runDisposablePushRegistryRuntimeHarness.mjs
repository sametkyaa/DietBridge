...
const runFlows = async () => {
  ...
  assert(sourceMigrations.at(-1) === '20260830185101_meal_plan_snapshot_edit_contract.sql', 'CANONICAL_MEAL_PLAN_SAVE_MIGRATION_TAIL');
  ...
  const runtimeFiles = readdirSync(runtimeMigrationDirectory)
    .filter((name) => /^\d+_.+\.sql$/.test(name))
    .sort();
  assert(runtimeFiles.length === 54, 'DISPOSABLE_MIGRATION_FILES_54');
  ...
  assert(countBySql('select count(*) from supabase_migrations.schema_migrations;') === 54, 'DISPOSABLE_MIGRATION_REPLAY_53_PLUS_PREREQUISITE');
