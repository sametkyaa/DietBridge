...
const runFlows = async () => {
  ...
  assert(sourceMigrations.at(-1) === '20260830141202_meal_plan_cross_day_identity_preservation.sql', 'CANONICAL_MEAL_PLAN_SAVE_MIGRATION_TAIL');
  ...
  const runtimeFiles = readdirSync(runtimeMigrationDirectory)
    .filter((name) => /^\d+_.+\.sql$/.test(name))
    .sort();
  assert(runtimeFiles.length === 53, 'DISPOSABLE_MIGRATION_FILES_53');
  ...
  assert(countBySql('select count(*) from supabase_migrations.schema_migrations;') === 53, 'DISPOSABLE_MIGRATION_REPLAY_52_PLUS_PREREQUISITE');
