...
assert(files.length === 56, 'MIGRATION_INVENTORY_56_WITH_CLIENT_GROCERY_LIST');
assert(files.includes(migrationName), 'PUSH_MIGRATION_REMAINS_CANONICAL');
assert(files.at(-8) === '20260826133224_product_admin_dietitian_verification.sql', 'PRODUCT_ADMIN_MIGRATION_BEFORE_STANDALONE_ADMIN');
assert(files.at(-7) === '20260827084741_standalone_platform_admin_access.sql', 'STANDALONE_ADMIN_MIGRATION_BEFORE_DIPLOMA_STORAGE');
assert(files.at(-6) === '20260830060342_dietitian_diploma_storage_hardening.sql', 'DIPLOMA_STORAGE_MIGRATION_BEFORE_MEAL_PLAN_SAVE');
assert(files.at(-5) === '20260830141202_meal_plan_cross_day_identity_preservation.sql', 'CROSS_DAY_MEAL_PLAN_SAVE_BEFORE_SNAPSHOT_EDIT');
assert(files.at(-4) === '20260830185101_meal_plan_snapshot_edit_contract.sql', 'SNAPSHOT_EDIT_BEFORE_NEW_RECIPE_CUSTOM_SNAPSHOT');
assert(files.at(-3) === '20260831071948_meal_plan_new_recipe_custom_snapshot_contract.sql', 'NEW_RECIPE_CUSTOM_SNAPSHOT_BEFORE_MEAL_COMPLETION_PHOTO');
assert(files.at(-2) === '20260831190352_meal_completion_photo_contract.sql', 'MEAL_COMPLETION_PHOTO_BEFORE_GROCERY_LIST');
assert(files.at(-1) === '20260901083212_client_grocery_list.sql', 'CLIENT_GROCERY_LIST_MIGRATION_IS_TAIL');
