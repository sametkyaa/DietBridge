...
- **Aşama durumları:** 3E-2C-2A COMPLETED; 3E-2C-2B production read-only contract audit COMPLETED / NOT READY; 3E-2C-2C pre-policy reconciliation package PREPARED; 3E-2C-2D reconciliation preflight PENDING; 3E-2C-2E reconciliation application BLOCKED; 3E-2C-2F RPC production smoke test BLOCKED; 3E-2C-3 legacy policy removal BLOCKED; Aşama 3 devam ediyor.
...
- **Production rollout blocker:** Production migration history ve verification consistency constraint eksiktir; üç kritik tabloda RLS kapalı, 11 kritik policy eksik, dört function search path drift'i vardır ve `set_my_meal_completion` RPC mevcut değildir. Dört ek policy manual review bekliyor; production RPC smoke testi yapılmadı. Reconciliation paketi yalnız hazırlandı, uygulanmadı; legacy policy korunuyor.
- **Sıradaki işlem:** Aşama 3E-2C-2D — Production pre-policy reconciliation preflight SQL'ini salt-okunur çalıştır; ana reconciliation SQL'ini çalıştırma.
- **Durum:** Devam ediyor.
...
| 3 | Supabase ve RLS | Devam ediyor | `codex/supabase-security` | 2026-07-13 |  | 3E-2C-2B audit COMPLETED / NOT READY; 3E-2C-2C reconciliation package PREPARED; 3E-2C-2D read-only preflight PENDING |
