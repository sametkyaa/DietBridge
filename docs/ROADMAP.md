...
- **Production rollout blocker:** Bir onaylı disposable diyetisyen, iki aktif client ilişkisi ve aynı diyetisyene bağlı planlar üreten mobil uyumlu fixture düzeltmesi hazırdır. Fiziksel mobil retry henüz yapılmadı; üç manuel mobil kapı tamamlanana kadar `POLICY_REMOVAL_ALLOWED=NO` ve legacy policy removal `BLOCKED` kalır.
- **Sıradaki işlem:** Ayrı manuel onayla mobil uyumlu production fixture yaşam döngüsünü çalıştır, CLI regresyonunu koru, fiziksel mobil retry’da own toggle/persistence/foreign-not-exposed sonuçlarını doğrula ve ağsız `mobile-confirm` ile kaydet. Bu görevde fixture veya policy removal çalıştırılmadı.
- **Durum:** Devam ediyor.
...
| 3 | Supabase ve RLS | Devam ediyor | `codex/supabase-security` | 2026-07-13 |  | Reconciliation COMPLETED; postflight PASSED; RPC smoke fixture PREPARED / NOT RUN; mobile production test PENDING; policy removal BLOCKED |
