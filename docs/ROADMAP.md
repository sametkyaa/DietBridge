...
- **WP5.4 zorunlu browser kabul blocker'ı:** Çalışma ortamı politikası sistem Chrome/Edge CDP ve local Vite process başlatmayı engellediği için WP5.3A browser runtime kabulü PASS sayılmadı. Aşama 5 tamamlanmadan önce active/pending/removed danışan görünürlüğü, hesap-izole localStorage, loading/error/empty/retry, geciktirilmiş stale client/hafta response'ları, save success/error state'i, 1440×900/1024×768/390×844 responsive davranışı ve 44×44 px etkileşim hedefleri browserda doğrulanmalıdır.
- **WP5.3C açık maddesi:** Mobil istemci, DB'de saklanan canonical meal-photo object path'ini private bucket için kısa süreli signed URL'ye çevirmeli; mobil plan okuma ve signed-photo sözleşmesi ayrıca doğrulanmalıdır. Queue-owner Storage delete P2 limitation'ı korunur; bu WP5.3B kapsamında değiştirilmedi.
- **Durum:** Devam ediyor — WP5.4 browser kabul blocker'ı çözülmeden Aşama 5 tamamlandı olarak işaretlenemez.
