...
| Appointments | Web uses canonical text status (`upcoming`, `completed`, `cancelled`) and `date`/`time`; ownership is dietitian/client scoped. | Mobile exposes a read-only appointment list/detail service surface scoped to the authenticated client; it does not create, update, delete, cancel, or reschedule appointments. | Both clients read the same appointment rows; Web remains the mutation owner and Mobile does not invent booking or lifecycle writes. |
