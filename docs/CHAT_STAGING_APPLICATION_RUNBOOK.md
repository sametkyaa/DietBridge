...
`supabase/tests/chat_security_harness.sql`, canonical migration zincirinin sekiz
dosyasını yukarıdaki sırayla varsayar. Harness tamamlandığında tam olarak 71
benzersiz `PASS:` etiketi ve tek `CHAT_SECURITY_HARNESS_PASS` marker'ı bulunmalı;
herhangi bir `FAIL:` marker'ı veya beklenmeyen SQL hatası başarısızlıktır.
