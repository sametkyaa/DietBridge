...
test('materializer creates exact 27+5 temp copies and an external manifest', async (t) => {
  ...
  assert.deepEqual(runtime.expectedHistory, { canonical: 27, image: 5, total: 32 });
  assert.equal(runtime.files.length, 32);
