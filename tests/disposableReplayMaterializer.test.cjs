...
test('materializer creates exact 29+7 temp copies and an external manifest', async (t) => {
  ...
  assert.deepEqual(runtime.expectedHistory, { canonical: 29, image: 7, total: 36 });
  assert.equal(runtime.files.length, 36);
