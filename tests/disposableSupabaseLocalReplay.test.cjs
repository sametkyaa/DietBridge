...
test('materializes the exact current migration chain in deterministic order', async (t) => {
  ...
  assert.deepEqual(result.manifest.expectedHistory, { canonical: 33, image: 7, total: 40 });
  assert.equal(result.manifest.files.length, 40);
  assert.equal(result.disposableHistory.repositoryMigrationCount, 40);
  assert.equal(result.disposableHistory.localPrerequisiteCount, 1);
  assert.equal(result.disposableHistory.disposableMigrationCount, 41);
  assert.deepEqual(
...
test('repository migration order remains unchanged inside the 40-entry disposable history', async (t) => {
  const result = await runMaterializeOnly({ keepTemp: true });
  t.after(() => rmSync(result.tempRoot, { recursive: true, force: true }));
